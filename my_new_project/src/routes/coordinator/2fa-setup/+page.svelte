...
<svelte:head>
	<title>אימות דו-שלבי לרכז</title>
	...
	<div class="w-full max-w-md bg-[#0f172a] rounded-3xl border border-white/10 shadow-2xl overflow-hidden">
		...
		<div class="p-8">
			<div class="text-center mb-6">
				...
				<h1 class="text-xl font-black text-white mb-1">אימות דו-שלבי לרכז</h1>
				<p class="text-gray-400 text-sm">הגנה נוספת על פאנל הרכז שלך</p>
			</div>
				...
				<ol class="text-gray-300 text-sm space-y-4">
					<li>
						...
						<button
							type="button"
							onclick={copySecret}
							class="w-full mt-2 font-mono text-cyan-300 text-sm bg-[#1e293b] border border-white/10 rounded-lg px-3 py-2 break-all hover:border-cyan-400/50 cursor-pointer"
						>
							{data.secret}
					...
					<li>
						...
						<form method="POST" action="?/enable" use:enhance class="mt-2 flex gap-2">
							...
							<input
								...
								maxlength="6"
								placeholder="000000"
								class="flex-1 text-center tracking-widest font-bold bg-[#1e293b] border border-white/10 rounded-lg px-3 py-2 text-white placeholder-gray-600 focus:outline-none focus:border-cyan-500"
							/>
							<button type="submit" class="px-4 rounded-lg bg-cyan-500/20 border border-cyan-500/40 text-cyan-200 font-bold text-sm hover:bg-cyan-500/30 cursor-pointer">
								הפעל
							</button>
						</form>
						{#if form?.done && !form.ok}
							<p class="mt-2 text-sm text-red-400">{form.message}</p>
						{/if}
						<p class="text-cyan-300/70 text-xs mt-2">💡 הסוד נשמר מוצפן בחשבונך — אין צורך להגדיר שום דבר ב-Vercel.</p>
					</li>
				</ol>
			{/if}

			<div class="text-center mt-6">
				<a href="/coordinator" class="text-gray-500 hover:text-gray-400 text-sm">← חזרה לפאנל הרכז</a>
			</div>
		</div>
	</div>
</div>
