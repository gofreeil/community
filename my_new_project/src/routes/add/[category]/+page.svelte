<script lang="ts">
    ...
    let savedId         = $state('');
    // כרטיס פנויים שנשמר בלי שינוי תמונות נשאר מאושר - בלי "עובר בדיקת צניעות"
    ...
    async function handleSubmit(e: Event) {
        ...
        try {
            ...
            savedPending = result.pending !== false;
            ...
            if (nbNotFound && customNb.trim()) {
                await submitNeighborhoodRequest({
                    ...
                    requesterName: getFieldValue('contact'), requesterPhone: getFieldValue('phone'),
                });
            }
