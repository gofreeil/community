...
export const POST: RequestHandler = async (event) => {
    ...
    try {
        const requests = await getItemsByCategory('shabbat_request');
        ...
        if (ef.host_user_id !== (session.user!.id as string)) {
            return json({ success: false, message: 'אין לך הרשאה לאשר בקשה זו' }, { status: 403 });
        }

        await updateItem(request_item_id, {
            extra_fields: { ...ef, status: action },
        });

        return json({ success: true });
    } catch (e) {
        console.error('[shabbat-approve POST]', e);
