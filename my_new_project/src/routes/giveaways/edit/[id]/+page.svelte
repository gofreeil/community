...
<svelte:head>
    <title>השלמת פרסום - {data.item.label} | קהילה בשכונה | יוצאים לחירות</title>
</svelte:head>
        ...
        <form method="POST" class="rounded-2xl bg-[#0f172a] border border-white/10 p-6 space-y-5">
            <div>
                ...
                {:else}
                    ...
                    <div class="mt-2">
                        <CameraCapture onfiles={processFiles} class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-yellow-500/40 hover:border-yellow-500/70 bg-yellow-900/10 hover:bg-yellow-900/20 text-yellow-100 text-sm font-bold transition-all cursor-pointer" />
                    </div>
                {/if}
