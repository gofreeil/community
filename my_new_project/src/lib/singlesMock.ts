...
export type UserStatus = 'available' | 'not_available' | 'in_relationship' | 'taking_break';
...
export function statusLabel(status: UserStatus | undefined): string {
    if (!status || status === 'available') return '✅ זמין/ה';
    if (status === 'not_available') return '🔴 לא זמין/ה';
    if (status === 'in_relationship') return '💑 בקשר';
    if (status === 'taking_break') return '⏸️ הפסקה זמנית';
    return '✅ זמין/ה';
}
