...
export interface FieldDef {
    key: string;
    label: string;
    type: 'text' | 'tel' | 'textarea' | 'select' | 'toggle' | 'checkbox' | 'number' | 'time' | 'date' | 'email' | 'availability_grid' | 'opening_hours' | 'multi_select' | 'neighborhood_select' | 'images' | 'map_pin' | 'address' | 'service_type';
    required: boolean;
    placeholder?: string;
    options?: string[];
    hint?: string;
...
export const categoryConfig: Record<string, CategoryConfig> = {
    ...
    singles: {
        ...
        fields: [
            ...
            { key: 'looking_for_m', label: 'מחפש',                  type: 'textarea', required: false, placeholder: 'למשל: רגישה, חכמה, יראת שמיים, עם חוש הומור...', maxLength: 1000, hint: '👁️ הטקסט כאן מוצג לכל מי שהכרטיס משותף איתו. בהמשך הטופס, בתא "מידע לשדכנים", אפשר להרחיב - וזה יגיע לשדכנים בלבד.', showIf: { field: 'gender', equals: 'גבר' } },
            { key: 'looking_for_f', label: 'מחפשת',                 type: 'textarea', required: false, placeholder: 'למשל: רגיש, חכם, ירא שמיים, עם חוש הומור...',  maxLength: 1000, hint: '👁️ הטקסט כאן מוצג לכל מי שהכרטיס משותף איתו. בהמשך הטופס, בתא "מידע לשדכנים", אפשר להרחיב - וזה יגיע לשדכנים בלבד.', showIf: { field: 'gender', equals: 'אישה' } },
            ...
            { key: 'visibility',  label: 'מי יראה את הכרטיס?',      type: 'toggle',   required: false, options: ['גלוי לכולם', 'רק לשדכנים שלנו'], default: 'רק לשדכנים שלנו', hint: '🔒 ברירת המחדל: “רק לשדכנים שלנו” — הכרטיס מוסתר מכולם; רק צוות השדכנים שלנו רואה אותו ומציע לך התאמות בדיסקרטיות. “גלוי לכולם” — הכרטיס יופיע בלוח הפומבי לצד הנגדי (גברים רואים נשים ולהפך).' },
            ...
            { key: 'contact',     label: 'שם (לא מוצג)',              type: 'text',     required: true,  placeholder: 'שם מלא ליצירת קשר', half: true, hint: 'השם יישמר פנימית ולא יוצג לציבור' },
            { key: 'phone',       label: 'טלפון (לא מוצג)',           type: 'tel',      required: true,  placeholder: '05X-XXXXXXX', half: true, hint: 'הטלפון יישמר פנימית ולא יוצג לציבור' },
            // ---- תא "מידע לשדכנים" (group=matchmakers) - נאסף בתחתית הטופס, לא מוצג ברבים ----
            { key: 'match_about_me', label: 'קצת עליי בהרחבה - לשדכנים בלבד', type: 'textarea', required: false, placeholder: 'מי אני באמת: אופי, מה חשוב לי בחיים, איך נראה יום רגיל שלי, מה מרגש אותי...', group: 'matchmakers' },
            { key: 'match_family_background', label: 'רקע משפחתי', type: 'textarea', required: false, placeholder: 'הורים, אחים ואחיות, מאיפה המשפחה, האווירה בבית...', group: 'matchmakers' },
            { key: 'match_children', label: 'ילדים (אם יש)', type: 'textarea', required: false, placeholder: 'גילאים, הסדרי משמורת, מה חשוב שבן/בת הזוג ידע/תדע', group: 'matchmakers' },
            { key: 'match_education_path', label: 'מסלול לימודים ושירות', type: 'textarea', required: false, placeholder: 'ישיבה / מדרשה / צבא / שירות לאומי / אקדמיה - איפה ומתי', group: 'matchmakers' },
            { key: 'match_religious_life', label: 'אורח החיים הדתי שלי בפועל', type: 'textarea', required: false, placeholder: 'תפילות, לימוד תורה, שבת, לבוש / כיסוי ראש - מה קבוע ומה גמיש', group: 'matchmakers' },
            { key: 'match_partner_religious', label: 'רמת הדתיות המבוקשת בבן/בת הזוג', type: 'textarea', required: false, placeholder: 'מה חשוב לך שיהיה משותף, ועד כמה אתה/את גמיש/ה', group: 'matchmakers' },
            { key: 'match_partner_character', label: 'מהו אופי בן הזוג שאתה מחפש',      type: 'textarea', required: false, group: 'matchmakers' },
            { key: 'match_age_range', label: 'טווח גילאים מבוקש', type: 'textarea', required: false, placeholder: 'למשל: 24-30', group: 'matchmakers' },
            { key: 'match_location', label: 'איפה תרצה/י לגור אחרי החתונה', type: 'textarea', required: false, placeholder: 'עיר / אזור, וכמה זה גמיש', group: 'matchmakers' },
            { key: 'match_life_goals', label: 'תוכניות ושאיפות לעתיד', type: 'textarea', required: false, placeholder: 'קריירה, לימודים, משפחה, חלומות...', group: 'matchmakers' },
            { key: 'match_self_advantage',    label: 'מהו היתרון שאתה מביא אל הזוגיות ?', type: 'textarea', required: false, group: 'matchmakers' },
            { key: 'match_past_experience', label: 'ניסיון קודם בהיכרויות - מה עבד ומה לא', type: 'textarea', required: false, placeholder: 'הצעות או קשרים קודמים, למה נגמרו, מה למדת על עצמך', group: 'matchmakers' },
            { key: 'match_dealbreakers', label: 'מה לא בא בחשבון אצלך', type: 'textarea', required: false, placeholder: 'דברים שאינך מתפשר/ת עליהם: עישון, מקום מגורים, רמת דתיות...', group: 'matchmakers' },
            { key: 'match_references', label: 'ממליצים (שם + טלפון)', type: 'textarea', required: false, placeholder: 'רב / חבר/ה קרוב/ה / מעסיק - מי שמכיר אותך ויכול לספר עליך', group: 'matchmakers' },
            { key: 'match_availability', label: 'זמינות לשיחה עם שדכן/ית', type: 'textarea', required: false, placeholder: 'ימים ושעות נוחים, טלפון או וואטסאפ', group: 'matchmakers' },
            { key: 'match_notes', label: 'משהו נוסף שחשוב שהשדכנים ידעו', type: 'textarea', required: false, placeholder: 'בריאות, נסיבות מיוחדות, בקשות לדיסקרטיות...', group: 'matchmakers' },
        ],
    },
